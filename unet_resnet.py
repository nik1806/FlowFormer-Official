...
class UNet(nn.Module):
    ...
    def __init__(self,  n_channels, n_classes, depth=6):
        """
        Args:
            n_classes: total number of classes(output) for dataset/model
        """
        super().__init__()
        self.n_channels = n_channels # input channels
        self.n_classes = n_classes # output classes
        self.DEPTH = depth
        ...
        self.input_block = nn.Sequential(
                                        nn.Conv2d(n_channels, 64, 7, 2, 3, bias=False),
                                        nn.BatchNorm2d(64),
                                        nn.ReLU(),
